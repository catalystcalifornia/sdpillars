...
This section focuses on stops that resulted in a field interview card being completed.

Look at:  maybe total time spent on them where only result was a fi card?, stop reason for original stop, race, age, stops by police beat
...
# create table
  ...
  flextable(df) %>% 
colformat_double(j = c("Total", "Count", "Percent"), digits = 1)%>% 
  set_caption('Officer Assignments that make Stops Resultinh in a Field Interview Card Alone or in Combination with Warning') %>% 
  theme_vanilla()%>%
          width(j=3,2,unit="in")
