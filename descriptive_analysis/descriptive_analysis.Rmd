...
This table looks at the most common stop reasons that result in a field interview card being completed. The majority of stops resulting in a field card interview are for __Reasonable Suspicion__ stops, at 73%, followed by __Traffic Violation__ stops.
...
__Gang enforcement__ and __Other__ police assignments conduct a high count of stops that result in a field interview card being completed. 
...
Nearly 1/4th of people who are stopped with a resulting field interview card being completed are age 25-34 or 35-44. 
...
Out of all people that are stopped with a resulting field interview card being completed, over 1/3 are Latinx people, and over 1/4 are Black people.
...
# create table
  ...
  flextable(df) %>% 
colformat_double(j = c("Total", "Count", "Percent"), digits = 1)%>% 
  set_caption('Race of People Stopped and Receiving a Field Interview Card') %>% 
  theme_vanilla()%>%
          width(j=4,3,unit="in")
...
This map visualizes the percentage of officer-initiated stops that result in a field interview card across SDPD police beats and divisions. Beats in the Southern region of San Diego have a higher rate of stops resulting in a field interview card being completed.
