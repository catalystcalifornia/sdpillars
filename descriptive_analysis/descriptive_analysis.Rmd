...
Look at: race, age, stops by police beat

## Total Number of Officer-Initiated Stops

In 2022, __10,663__ officer-initiated stops resulted in a field interview card.  This accounts for 12.1% of all officer-initiated stops in 2022 across all police units. There were __1,407__ officer-initiated stops that resulted in a field interview card and a warning. 

Interestingly, there were 0 calls for service that resulted in a field interview card being completed. This only results from an officer-initiated stop. 
...
Time spent on stops where the only result was a field interview card. Note that this is currently using the raw stop duration times, and can be replaced with the stop duration times with outliers capped. 
...
Most common stop reasons that resulted in a field interview card being completed 
...
Police assignment of officers that most commonly conduct stops that resulted in a field interview card being completed 
...
Age of people stopped that resulted in a field interview card
...
Race of people stopped that resulted in a field interview card
...
df<-field%>%
  ...
  mutate(Count=n(),
         Percent=Count/Total*100)%>%
  mutate(Year='2022')%>%
  select(Year, nh_race, result_list, Total, Count, Percent)%>%
  slice(1)%>%
  rename('Race'='nh_race')%>% 
  arrange(-Percent)
...
df1<-field%>%
  ...
  summarise(Year,
   ...
   pivot_longer(2:4, names_to = "Race", values_to="Total" )%>%
   group_by(Race)%>%
  slice(1)
...
df2<-field%>%
  left_join(race)%>%
  select(Year, result_list, sswana_flag, aian_flag, nhpi_flag)%>%
   filter(result_list %in% c("Field interview card completed","Field interview card completed, Warning (verbal or written)"))%>%
  group_by()
  mutate(Count=n())%>%
...
Map looking at officer-initiated stops that result in a field interview alone or in combination with a warning across SDPD police beats.
