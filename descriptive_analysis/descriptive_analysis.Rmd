...
This section focuses on stops that resulted in a field interview card being completed either alone or in combination with a warning result.

## Total Number of Officer-Initiated Stops

In 2022, __10,663__ officer-initiated stops resulted in a field interview card.  This accounts for 12.1% of all officer-initiated stops across all police units. There were __1,407__ officer-initiated stops that resulted in a field interview card and a warning. There were 0 calls for service that resulted in a field interview card being completed. 
...
This table looks at the most common stop reasons that result in a field interview card being completed alone or in combination with a warning result. More than 20% of stops for __Known to be on Parole / Probation / PRCS / Mandatory Supervision__ or __Reasonable Suspicion__ result in a field interview card being completed. 
...
df<-field%>%
  ...
  mutate(Total=n())%>%
  filter(result_list %in% c("Field interview card completed","Field interview card completed, Warning (verbal or written)"))%>%
  group_by(reason_simple, result_list)%>%
  mutate(Count=n(),
         Percent=Count/Total*100)%>%
  mutate(Year='2022',
         Assignment='All units')%>%
  select(Year, Assignment, reason_simple, result_list, Total, Count, Percent)%>%
  slice(1)%>%
  rename('Stop Result'='result_list',
...
colformat_double(j = c("Total", "Count", "Percent"), digits = 1)%>% 
  set_caption('Stop Reasons that Result in Field Interview Card Alone or in Combination with Warning') %>% 
  theme_vanilla()%>%
      width(j=3,2,unit="in")%>%
          width(j=4,2,unit="in")
...
__Gang enforcement__ and __Other__ police assignments conduct a high count of stops that result in a field interview card being completed alone or in combination with a warning result. 
...
df<-field%>%
  ...
  mutate(Total=n())%>%
  filter(result_list %in% c("Field interview card completed","Field interview card completed, Warning (verbal or written)"))%>%
  group_by(assignment, result_list)%>%
  mutate(Count=n(),
         Percent=Count/Total*100)%>%
  mutate(Year='2022')%>%
  select(Year, assignment, result_list, Total, Count, Percent)%>%
  slice(1)%>%
  rename('Assignment'='assignment')%>% 
  arrange(result_list, -Percent)

# create table

  flextable(df) %>% 
colformat_double(j = c("Total", "Count", "Percent"), digits = 1)%>% 
  set_caption('Officer Assignments that make Stops Resulting in a Field Interview Card Alone or in Combination with Warning') %>% 
  theme_vanilla()%>%
          width(j=3,3,unit="in")
...
The age distribution of people who are stopped with a resulting field interview card alone or in combination with a warning result is somewhat consistent. Over 14% of people age 18 and under are being stopped and having a field interview card completed. 
...
df<-field%>%
  ...
  mutate(Total=n())%>%
  filter(result_list %in% c("Field interview card completed","Field interview card completed, Warning (verbal or written)"))%>%
  group_by(age_bracket, result_list)%>%
  mutate(Count=n(),
         Percent=Count/Total*100)%>%
  mutate(Year='2022')%>%
  select(Year, age_bracket, result_list, Total, Count, Percent)%>%
  slice(1)%>%
...
# create table
  ...
  flextable(df) %>% 
colformat_double(j = c("Total", "Count", "Percent"), digits = 1)%>% 
  set_caption('Age of People Stopped and Receiving a Field Interview Card Alone or in Combination with Warning') %>% 
  theme_vanilla()%>%
          width(j=3,3,unit="in")
...
Black and Latinx people are disproportionately stopped with a field interview card being completed as a result, alone or in combination with a warning. There is also a high percentage of AIAN people being stopped and having a field interview card completed. 
...
df<-field%>%
  ...
  mutate(Total=n())%>%
  filter(result_list %in% c("Field interview card completed","Field interview card completed, Warning (verbal or written)"))%>%
  group_by(nh_race, result_list)%>%
  mutate(Count=n(),
         Percent=Count/Total*100)%>%
  mutate(Year='2022',
         Assignment='All units',)%>%
  select(Year, Assignment, nh_race, result_list, Total, Count, Percent)%>%
  slice(1)%>%
  rename('Race'='nh_race',
         'Result'='result_list')%>% 
  arrange(-Percent)
...
df1<-field%>%
  left_join(race)%>%
  select(Year, stop_id, person_id, result_list, sswana_flag, aian_flag, nhpi_flag)%>%
  pivot_longer(5:7, names_to = "Race", values_to="value" )%>%
 group_by(result_list, Race)%>%
   filter(result_list %in% c("Field interview card completed","Field interview card completed, Warning (verbal or written)"))%>%
 mutate(Count=sum(value))%>%
  slice(1)%>%
  mutate(Total=ifelse(Race %in% "aian_flag", 228,
                      ifelse(Race %in% "sswana_flag", 3052, 871)),
         Race=ifelse(Race %in% "aian_flag", "AIAN",
                      ifelse(Race %in% "sswana_flag", "SSWANA", "NHPI")),
         Percent=Count/Total*100,
         Assignment='All units')%>%
  rename('Result'='result_list')%>% 
  select(Year, Assignment, Race, Result, Total, Count, Percent )
...
# create table
  ...
  flextable(df) %>% 
colformat_double(j = c("Total", "Count", "Percent"), digits = 1)%>% 
  set_caption('Race of People Stopped and Receiving a Field Interview Card Alone or in Combination with Warning') %>% 
  theme_vanilla()%>%
          width(j=4,3,unit="in")
...
This map visualizes the percentage of officer-initiated stops that result in a field interview card alone or in combination with a warning across SDPD police beats and divisions. Beats in the Southern region of San Diego have a higher rate of stops resulting in a field interview card being completed.
...
#### Field interview alone OR IN COMBINATION with a warning layer ####

field_warning<-field%>%
  group_by(beat_name)%>%
  mutate(Total=n())%>%
  group_by(beat_name, result_list)%>%
filter(result_list =="Field interview card completed, Warning (verbal or written)")%>%
   mutate(Count=n(),
          Percent=Count/Total*100)%>%
  inner_join(beats_dissolve, by=c("beat"="beat"))%>%
  slice(1)%>%
...
field_warning<-st_as_sf(field_warning)%>%
  st_transform(4326)
...
pal_warning<-colorBin(
  palette =cc_brand,
  domain = field_warning$Percent,
  bins=c(0,1.5,3,Inf),
   na.color = NA
)
...
popup_warning<-paste0("Assignment: All Units","<br>","Beat Name: ",field_warning$beat_name,"<br>","Stop count: ", field_warning$Count,"<br>","Stop percent: ", percent(field_warning$Percent/100,.1))
...
labels_warning<- c( "0%-1.5%", "1.5%-3%", "Greater than 3%")
...
leaflet(width = "100%", height = "600px") %>%
  ...
  addMapPane("warning_pane", zIndex=485)%>%
  ...
  # Field interview + warning layer
  
    addPolygons(data = field_warning,  label=~htmlEscape(paste0("Beat name: ",field_warning$beat_name)), color = "#444444", weight = 1, smoothFactor = 0.5,  opacity = 1, fillOpacity =1,  fillColor = ~pal_warning(field_warning$Percent),
               popup = ~paste0(popup_warning),
              highlight = highlightOptions(color = "white", weight = 3, bringToFront = FALSE), group = "Field Interview Card and Warning",
              options = pathOptions(pane = "warning_pane")) %>%
 ...
 ## add legends for stop count for each layer
  ...
  addLegend(position = "bottomleft", pal = pal_warning, values = field_alone$Percent, opacity = 1,
            title = "Field Interview Card and Warning", 
            na.label = NA, 
            group="Field Interview Card and Warning",
             labFormat= function(type, cuts, p) {  
                                                  paste0(labels_warning)
                             }) %>%

  # layer control
  
   addLayersControl(overlayGroups = c("Field Interview Card (Alone)","Field Interview Card and Warning", "SDPD Divisions"),
                   options = layersControlOptions(collapsed = FALSE),
                   position = 'topright')%>%
  
  
  #hide supervisorial districts by default
  hideGroup(c( "SDPD Divisions","Field Interview Card and Warning"))%>%
