...
Before engaging in detailed analysis regarding the project’s research questions, we will first run general frequencies of SDPD stops to get a sense of overall trends. This helps us determine if our expected findings are likely to show up in the data and allows us to identify any trends we did not expect. The analysis will include: 
...
# First, confirm that within each stop_id and person_id, the reason for stop is the same. And that the reason for multiple rows is because of the other colunm values
...
# Now, work on flattening the reasons table so we get one person-one stop reason single row data
...
reason_p<-person%>%
  ...
  slice(1)%>%
  ungroup() #### I believe this gives us a single stop reason for each unique person
