...
Analyze, out of all stops that resulted in a search, what the resulting action of those stops were. Note there will be a slight over count because people can be searched for more than 1 search type, resulting in a double count. People who have been searched can also have more than one result. We are ok with the slight over count for now as we gauge general trends. 
...
mutate_all(na_if,"")%>%
  ...
  mutate(Total=sum(did_search_occur))
...
contra_p<-contra%>%
  ...
  mutate(search_total=21588)%>%
  group_by(contraband)%>%
  mutate(contra_count=n(),
         rate=contra_count/search_total*100)

# join both tables together

hitrate<-left_join(search_p, contra_p)
