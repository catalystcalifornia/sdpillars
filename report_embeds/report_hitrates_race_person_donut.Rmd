...
df %>%
  hchart(
    ...
    name = "",
    tooltip = list(headerFormat="", pointFormat = "<b>{point.rate:.1f}%</b> of people searched by SDPD had <b>{point.label}</b>, a total of <b>{point.count:,.0f}</b>  people needlessly searched."),
     innerSize="65%",
     center = c(50, 50), 
    )%>% 
 ...
 align="left")%>%
   hc_subtitle(text = "Percent of people searched by whether contraband or evidence was found",
                     align="left") %>%
  hc_caption(
    text = paste0("<br><br>", sourcenote, "<br>"," Analysis for all officer-initiated stops."))%>%
  hc_plotOptions(
    innersize="50%", 
    startAngle=90, 
    endAngle=90,
