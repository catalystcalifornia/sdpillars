...
# Important to note our "class" names (e.g., leaflet-popup-scrolled, division-header, stop-count-total, etc.) 
# <div> "class" names are used as references with CSS styling (end of script, before JS)
# <span> "class" names help us insert the data we have stored in data_popup
...
merged_rate_popup <- "<div class='leaflet-popup-scrolled point-density-sidebar' style='max-width:800px;max-height:200px'><div class='popup-instruction'><b>Please click on a police division to see stop data below.</b></div>

<div class ='division-header' style='font-size: 14px; font-weight: bold;'><span class='division-name'></span> Division (Division <span class='division-number'></span>)</div></div>

<br>

Out of <span class='stop-count-total' style='font-size: 14px; font-weight: bold;'></span> people searched during stops where a field interview card was completed:

<br><br>

SDPD found contraband/evidence on <span class='yes-contraband-count' style='font-size: 14px; font-weight: bold;'></span> people, a hit rate of <span class='yes-contraband-prc-rate' style='font-size: 14px; font-weight: bold;'></span>%.

<br><br>

SDPD found no contraband or evidence on <span class='no-contraband-count' style='font-size: 14px; font-weight: bold;'></span> people, meaning SDPD  subjected <span class='no-contraband-prc-rate' style='font-size: 14px; font-weight: bold;'></span>% of people to unnecessary searches.
</div>
...
map <- leaflet(width = "100%", height = "600px") %>%
  ...
  addPolygons(data = div_wide_sf, label=~htmlEscape(paste0(div_name, " Division ")),
              color = "black", fillColor = "white", opacity = 1,  popup = ~paste0(data_popup),
  ...
  addControl(html=merged_rate_popup, position = "topleft") %>%
...
```{js}
<!-- This function is used to format SDPD RIPA data in the point density map "popup"/sidebar feature -->
