...
tooltip_text <- "For every 100 <b>{point.race_label_short:.1f}</b> people in San Diego stopped for an officer-initiated stop, SDPD stopped <b>{point.rate:.1f}</b> people perceived as <b>{point.race_label_short:.1f}</b> for stops that resulted in  <b>{point.result_simple:.1f}</b>, a total of <b>{point.count:,0f}</b> people."
...
#   subtitle = 'Percent of officer-initiated stops that result in a warning or no action by race',
#   tooltip = tooltip_text,
#   caption = paste0(racenote,"<br><br>", sourcenote, "<br>","Analysis for all officer-initiated stops.","<br>"))


## have to do it manually to get the order to work
group_colors = group_colors_item
...
chart<-hchart(df, 
    ...
    hc_title(
      text = 'SDPD is most likely to stop Black and Latinx people for stops that then result in no action') %>%
    
    hc_subtitle(
      text = 'Percent of officer-initiated stops that result in a warning or no action by race') %>%
    
    hc_caption(
      text = paste0(racenote,"<br><br>", sourcenote, "<br>","Analysis for all officer-initiated stops.","<br>")) %>%
    
    hc_yAxis(title = list(text = paste0(""))) %>%
    
    hc_xAxis(categories = list("Black","Latinx", "AIAN", "Asian", "NHPI", "SWANA/SA", "Multiracial", "White"),
