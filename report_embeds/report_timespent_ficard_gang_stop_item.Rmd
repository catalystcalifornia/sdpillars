...
# create list of categories in preferred order
...
# create a factor column of the stop_result categories
df$ordered_labels <- as.factor(df$stop_result_simple_labels)
# set the factor levels with the ordered list above
levels(df$ordered_labels) <- ordered_labels_list
# confirm levels are in the right order
levels(df$ordered_labels)

# arrange the df according to factor levels
df <- df %>%
  arrange(ordered_labels)

# View(df)

# custom item coloring
# order colors to align with factor levels (if needed)
